...
class HLSrocc0_vaddControl(opcodes: OpcodeSet)(implicit p: Parameters) extends LazyRoCC(opcodes) {
  override lazy val module = new HLSrocc0_vaddControlModule(this)
}

class HLSrocc0_vaddControlModule(outer: HLSrocc0_vaddControl)(implicit p: Parameters) extends LazyRoCCModuleImp(outer) 
...
//===== Begin Accelerator =====
val accel = Module(new HLSrocc0_vaddBlackbox())
